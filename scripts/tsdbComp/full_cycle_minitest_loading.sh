...
if [ -f "${BULK_DATA_DIR}/${INSERT_DATA_FILE_NAME}" ]; then
    ...
    log_debug "Generating execute commod: ${EXE_FILE_NAME_GENERATE_DATA} \
...
if [ "${FORMAT}" == "timescaledb" ];then
    ...
    if [  ${FORMAT} == "influx" ]; then
        ...
        if ! run_command "source ${scriptDir}/logger.sh && source ${scriptDir}/common.sh && check_influxdb3_status ${DATABASE_PORT}"; then
