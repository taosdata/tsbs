...
function install_tsbs {
  ...
  cd ${installPath}/tsbs/cmd/tsbs_load_influx3/  &&  go build && cp tsbs_load_influx ${GOPATH}/bin/
  cd ${installPath}/tsbs/cmd/tsbs_run_queries_influx3/  &&  go build && cp tsbs_run_queries_influx ${GOPATH}/bin/
}
