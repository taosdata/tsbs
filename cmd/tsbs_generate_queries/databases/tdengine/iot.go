...
func (i *IoT) StationaryTrucks(qi query.Query) {
	...
	//select name,driver from (SELECT name,driver,fleet ,avg(velocity) as mean_velocity FROM readings WHERE ts > '2016-01-01T15:07:21Z' AND ts <= '2016-01-01T16:17:21Z' partition BY name,driver,fleet interval(10m) LIMIT 1) WHERE fleet = 'West' AND mean_velocity < 1000 partition BY name;
	sql := fmt.Sprintf("select name,driver from (SELECT name,driver,fleet ,avg(velocity) as mean_velocity FROM readings WHERE ts > %d AND ts <= %d partition BY name,driver,fleet interval(10m) LIMIT 1) WHERE fleet = '%s' AND mean_velocity < 1000 partition BY name;", interval.StartUnixMillis(), interval.EndUnixMillis(), i.GetRandomFleet())
