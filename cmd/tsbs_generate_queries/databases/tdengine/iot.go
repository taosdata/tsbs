...
func (i *IoT) AvgDailyDrivingSession(qi query.Query) {
	...
	interval := i.Interval.MustRandWindow(iot.StationaryDuration)
	sql := fmt.Sprintf("    select _wstart as ts,name,avg(ela) from (select ts,name,ela from (SELECT ts,name, diff(difka) as dif, diff(cast(ts as bigint)) as ela FROM (SELECT ts,name,difka  FROM (SELECT ts,name,diff(mv) AS difka  FROM (SELECT _wstart as ts,name,cast(cast(floor(avg(velocity)/5) as bool) as int) AS mv FROM readings   WHERE name is not null  AND ts > %d AND ts < %d   partition by name interval(10m) fill(value,0))partition BY name ) WHERE difka!=0   partition BY name ) partition BY name ) WHERE dif = -2   partition BY name )  partition BY name  interval(1d);", interval.StartUnixMillis(), interval.EndUnixMillis())
