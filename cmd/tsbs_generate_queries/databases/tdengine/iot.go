...
func (i *IoT) StationaryTrucks(qi query.Query) {
	...
	//select name,driver from (
	//	SELECT mean(velocity) as mean_velocity ,name,driver,fleet
	//	FROM readings
	//	WHERE ts > 1451606400000 AND ts <= 1451606460000
	//	partition BY name,driver,fleet interval(10m)
	//	LIMIT 1)
	//	WHERE fleet = '%s' AND mean_velocity < 1
	//	GROUP BY name
	sql := fmt.Sprintf("select name,driver from ( SELECT mean(velocity) as mean_velocity ,name,driver,fleet FROM readings WHERE ts > %d AND ts <= %d partition BY name,driver,fleet interval(10m) LIMIT 1) WHERE fleet = '%s' AND mean_velocity < 1 GROUP BY name", interval.StartUnixMillis(), interval.EndUnixMillis(), i.GetRandomFleet())
...
func (i *IoT) TrucksWithLongDrivingSessions(qi query.Query) {
	...
	//SELECT name,driver FROM(
	//	SELECT count(*) AS ten_min FROM(
	//	SELECT name,driver,avg(velocity) as mean_velocity
	//FROM readings
	//WHERE fleet ="South4" ts > 1451606400000 AND ts <= 1451606460000
	//Gpartition BY name,driver interval(10m))
	//WHERE mean_velocity > 1
	//GROUP BY name,driver)
	//WHERE ten_min_mean_velocity > %d
	sql := fmt.Sprintf("SELECT name,driver FROM( SELECT count(*) AS ten_min FROM( SELECT name,driver,avg(velocity) as mean_velocity FROM readings WHERE fleet =\"%s\" ts > %d AND ts <= %d Gpartition BY name,driver interval(10m)) WHERE mean_velocity > 1 GROUP BY name,driver) WHERE ten_min_mean_velocity > %d", i.GetRandomFleet(), interval.StartUnixMillis(), interval.EndUnixMillis(), tenMinutePeriods(5, iot.LongDrivingSessionDuration))
