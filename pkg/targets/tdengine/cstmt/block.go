...
func ReadBlockWithTimeFormat(block unsafe.Pointer, blockSize int, colTypes []uint8, precision int, formatFunc FormatTimeFunc) [][]driver.Value {
	...
	pHeader := uintptr(block) + payloadOffset + 12 + uintptr(6*len(colTypes)) + 12 // length i32, group u64
	pStart := pHeader
	for column := 0; column < colCount; column++ {
		colLength := *((*int32)(unsafe.Pointer(uintptr(block) + 12 + 12 + uintptr(6*len(colTypes)) + uintptr(column)*4)))
